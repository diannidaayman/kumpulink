...
- **Dua penyimpangan gaya yang disengaja, beserta alasannya.** Keduanya tetap terbuka untuk ditinjau: bila peninjau menemukan alasannya tidak berlaku, atau menemukan cacat lain di sekitarnya, temuan itu sah dan diputuskan pemilik — bukan dibungkam di muka.
...
- **TDD berlaku untuk Task 2 sampai 7**, yang memuat seluruh logika unit ini: empat fungsi murni, skema Zod, dua server action, dan route QR. Tulis pengujiannya lebih dulu, jalankan sampai merah, baru implementasikan. Task 1 dan 10 tidak memuat kode. Task 8 dan 9 membangun komponen React, dan `vitest.config.mts` berjalan di environment `node` tanpa jsdom maupun testing-library — keduanya diverifikasi lewat pemeriksaan peramban yang tertulis di dalamnya, cara yang sama yang dipakai Unit 2, 3, dan 4. Memasang fondasi pengujian komponen di tengah unit antarmuka bukan lingkup Unit 5. Ditetapkan pemilik di Pre-Flight Plan Review, 8 September 2026.
